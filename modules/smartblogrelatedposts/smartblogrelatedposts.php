    ...
    public function hookdisplaySmartAfterPost($params)
       {
        if(Module::isInstalled('smartblog') != 1){
	     ...
	     if (!$this->isCached('smartblogrelatedposts.tpl', $this->getCacheId()))
                        {
                        $id_cat  = BlogCategory::getCategoryNameByPost(doubleval(Tools::getvalue('id_post')));
                             $id_lang = $this->context->language->id;
                             $posts =  SmartBlogPost::getRelatedPosts($id_lang,$id_cat, doubleval(Tools::getvalue('id_post')));
                             $this->smarty->assign( array(
                                           'posts' => $posts
                                 ));
                        }
