...
include_once('categoryslider.php');

$context = Context::getContext();
$home_slider = new CategorySlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
	{
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'categoryslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_categoryslider_slides` = '.(int)$id_slide
		);
