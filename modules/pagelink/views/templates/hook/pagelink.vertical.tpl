{assign var='option_tpl' value=OvicLayoutControl::getTemplateFile('pagelink.vertical.tpl','pagelink')}
{if  $option_tpl!== null}
	     	...
	     	<ul class="dropdown-menu" role="menu">
	     		{foreach from=$menuContents item=parent_menu name=parent_menus}
					...
					<li class="{$parent_menu.custom_class}">
						<a href="#">{$parent_menu.currencies.name}<span class="dropdown-value">{$parent_menu.currencies.iso_code}</span></a>
			           <ul>
					...
					{elseif $parent_menu['link_type'] == 'LANGUAGE-BOX'}
						<li class="{$parent_menu.custom_class}">
							<a href="#">{$parent_menu.name}<span class="dropdown-value">{$lang_iso}</span></a>
							<ul>
