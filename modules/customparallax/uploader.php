...
require_once(dirname(__FILE__).'/customparallax.php');
$module = new CustomParallax();
			...
			if($uploadType == 'image'){
				if($width == null && $height == null){
					if (@move_uploaded_file($_FILES['uploader']['tmp_name'], $fileTemp)) {						
						...
						$response->status = 0;
						$response->msg = $module->l("Upload file not 1 success!");
						$response->fileName = "";
					}
				}else{
					if (@move_uploaded_file($_FILES['uploader']['tmp_name'], $fileTemp)) {					
						$imageSize = getimagesize($fileTemp);	
						include(dirname(__FILE__).'/class/CustomParallax.php');					
						$img = new CustomParallax();
						...
						$response->status = 0;
						$response->msg = $module->l("Upload file not 2 success!");
						$response->fileName = "";
					}	
				}
			}else{				
				if (@move_uploaded_file($_FILES['uploader']['tmp_name'], $fileTemp)) {
					...
					$response->status = 0;
					$response->msg = $module->l("Upload file not 3 success!");
					$response->fileName = "";
				}
			}
		}else{
