...
{if isset($p) AND $p}
	...
	<div id="pagination{if isset($paginationId)}_{$paginationId}{/if}" class="pagination pages pagination-container clear-margin clearfix" style="width: 100%">
	    {if $nb_products > $products_per_page && $start!=$stop}
			<form class="showall" action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get" style="display: none">
				<div>
			...
			<ul class="pagination pull-right">
				{if $p != 1}
					...
					<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous">
						<a class="prev-page" {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}"></a>
					</li>
				{else}
					<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous">
				...
				{if $pages_nb > 1 AND $p != $pages_nb}
					...
					<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next">						
						<a class="next-page" {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}"></a>
					</li>
				{else}
					<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next">						
