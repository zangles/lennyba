	...
	<div class="container">
		...
		<div class="xs-margin"></div>
        	...
        	{if !isset($empty) && !$PS_CATALOG_MODE}
					<div class="heading-counter warning hidden">{l s='Your shopping cart contains:'}
			...
			{else}	
				...
				<div id="order-detail-content" class="table_block table-responsive">
					<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
						<thead>
							<tr>
								<th class="cart_product first_item table-title" width="20%">{l s='Product'}</th>
								<th class="cart_description item table-title">{l s='Description'}</th>
								{*}{if $PS_STOCK_MANAGEMENT}{*}
									{assign var='col_span_subtotal' value='3'}
									<th class="cart_avail item text-center table-title">{l s='Availability'}</th>
								{*}
								{else}
									{assign var='col_span_subtotal' value='2'}
								{/if}
								{*}
								<th class="cart_unit item text-right table-title">{l s='Unit price'}</th>
								<th width="130" class="cart_quantity item text-center table-title">{l s='QUANTITY'}</th>
								
								<th class="cart_total item text-right table-title">{l s='Total'}</th>
								<th class="cart_delete last_item table-title"><span class="close-button disabled"></span></th>
							</tr>
						</thead>
						<tfoot>
							...
							{if $use_taxes}
								{if $priceDisplay}
									<tr class="cart_total_price">
										...
										<td colspan="2" class="price" id="total_product"><span class="product-price-special">{displayPrice price=$total_products}</span></td>
									</tr>
								{else}
									<tr class="cart_total_price">
										<td rowspan="5" colspan="2" id="cart_voucher" class="cart_voucher">
											{if $voucherAllowed}
												{if isset($errors_discount) && $errors_discount}
													<ul class="alert alert-danger">
														{foreach $errors_discount as $k=>$error}
										...
										<td colspan="{$col_span_subtotal}" class="text-left">{if $display_tax_label}{l s='Total products (tax incl.)'}{else}{l s='Total products'}{/if}</td>
										<td colspan="2" class="price" id="total_product"><span class="product-price-special">{displayPrice price=$total_products_wt}</span></td>
									</tr>
								{/if}
							{else}
								<tr class="cart_total_price">
									...
									<td colspan="{$col_span_subtotal}" class="text-left">{l s='Total products'}</td>
									<td colspan="2" class="price" id="total_product product-price-special">{displayPrice price=$total_products}</td>
								</tr>
							{/if}
							<tr>
							...
							{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
								<tr class="cart_total_delivery{if !$opc && (!isset($cart->id_address_delivery) || !$cart->id_address_delivery)} {/if}">
							...
							{else}
								{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
									{if $priceDisplay}
										<tr class="cart_total_delivery{if $total_shipping_tax_exc <= 0} {/if}">
									...
									{else}
										<tr class="cart_total_delivery{if $total_shipping <= 0} {/if}">
								...
								{/if}
							{/if}
							<tr class="cart_total_voucher{if $total_discounts == 0} {/if}">
							...
							{if $use_taxes && $show_taxes && $total_tax != 0 }
								...
								<tr class="cart_total_price">
									...
									<td colspan="2" class="price" id="total_price_without_tax"><span class="product-price-special">{displayPrice price=$total_price_without_tax}</span></td>
								</tr>
								{/if}
								<tr class="cart_total_tax">
									<td colspan="{$col_span_subtotal}" class="text-left">{l s='Tax'}</td>
									<td colspan="2" class="price" id="total_tax"><span class="product-price-special">{displayPrice price=$total_tax}</span></td>
								</tr>
							{/if}
							<tr class="cart_total_price">
								...
								{if $use_taxes}
									<td colspan="2" class="price product-total-col" id="total_price_container ">
										<span class="product-price-special" id="total_price">{displayPrice price=$total_price}</span>
									</td>
								{else}
									<td colspan="2" class="price product-total-col" id="total_price_container">
										<span class="product-price-special" id="total_price">{displayPrice price=$total_price_without_tax}</span>
									</td>
								{/if}
							</tr>
						</tfoot>
							...
							{if $delivery->id}
								<div class="col-xs-12 col-sm-6"{if !$have_non_virtual_products} style="display: none;"{/if}>
									<ul id="delivery_address" class="address item box">
										<li><h2 class="color2">{l s='Delivery address'}&nbsp;<span class="address_alias">({$delivery->alias})</span></h2></li>
							...
							{if $invoice->id}
								<div class="col-xs-12 col-sm-6">
									<ul id="invoice_address" class="address alternate_item box">
										<li><h2 class="color2">{l s='Invoice address'}&nbsp;<span class="address_alias">({$invoice->alias})</span></h2></li>
