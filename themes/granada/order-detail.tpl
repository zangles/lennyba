	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6">
			<ul class="address item {if $order->isVirtual()}full_width{/if} box">
				<li><h2 class="color2">{l s='Invoice address'} ({$address_invoice->alias})</h2></li>
...
<div id="order-detail-content" class="table_block table-responsive">
	<table class="table table-bordered">
		<thead>
			<tr>
				{if $return_allowed}<th class="first_item table-title"><input type="checkbox" /></th>{/if}
				<th class="{if $return_allowed}item{else}first_item{/if} table-title">{l s='Reference'}</th>
				<th class="item table-title">{l s='Product'}</th>
				<th class="item table-title">{l s='Quantity'}</th>
				{if $order->hasProductReturned()}
					<th class="item table-title">{l s='Returned'}</th>
				{/if}
				<th class="item table-title">{l s='Unit price'}</th>
				<th class="last_item table-title">{l s='Total price'}</th>
			</tr>
		</thead>
		<tfoot>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button btn btn-default button-medium button_111_hover"><span>{l s='Send'}<i class="icon-chevron-right right"></i></span></button>
		</div>
	</form>
{else}
