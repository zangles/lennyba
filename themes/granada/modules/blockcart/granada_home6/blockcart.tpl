...
{/if}
						...
						<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
							...
							<div class="clearfix">
								...
								<ul class="pull-right action-btn-container">
									<li><a class="btn btn-custom-5 button_111_hover" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">{l s='Cart' mod='blockcart'}</a></li>
									<li><a class="btn btn-custom second_button_111_hover" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">{l s='Checkout' mod='blockcart'}</a></li>
								</ul>
							 </div>
    ...
    {if !$PS_CATALOG_MODE && $active_overlay == 1}
    	...
    	<div class="layer_cart_overlay full-size"></div>
    {/if}