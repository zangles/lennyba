...
*}
<!-- MODULE Block cart -->
{assign var='option_tpl' value=OvicLayoutControl::getTemplateFile('blockcart.tpl','blockcart')}
{if  $option_tpl!== null}
    ...
    {/if}
    	<div class="shopping_cart">
    		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
    			{*}<span class="cart_name_block">{l s='Cart:' mod='blockcart'}</span>{*}
    			<span class="ajax_cart_quantity">{$cart_qties}</span>
    			<span class="ajax_cart_product_txt_empty{if $cart_qties != 0} unvisible{/if}">{l s='item(s)' mod='blockcart'}&nbsp;-&nbsp;</span>
    		...
    		{if !$PS_CATALOG_MODE}
    			...
    			<div class="cart_block block exclusive ">
    				...
    				<div class="block_content">
    					...
    					<div class="cart_block_list clearfix{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
    						{if $products}
    							<dl class="products">
    									...
    									<dt data-id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery}{else}0{/if}" class="mainHoverColor {if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
    										...
    										<div class="cart-info">
    											<div class="product-name">
    												<a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:30:'...'|escape:'html':'UTF-8'}</a>
    											</div>
    						...
    						<p class="cart-buttons">
                                <a href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
    									{l s='Cart' mod='blockcart'}
    							</a>
    							<a id="button_order_cart" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
