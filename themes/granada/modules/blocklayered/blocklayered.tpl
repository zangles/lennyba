...
<div id="layered_block_left" class="widget">
	<p class="title_block hidden">{l s='Catalog' mod='blocklayered'}</p>
			...
			<div class="accordion"  id="sidebar-collapse-filter">
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						{if isset($filter.slider)}
							<div class="accordion-group panel layered_filter layered_{$filter.type}" style="display: none;">
						{else}
							<div class="accordion-group panel layered_filter">
						{/if}
						<div class="accordion-title">{$filter.name|escape:'html':'UTF-8'} <a class="accordion-btn open" data-toggle="collapse" href="#div_layered_{$filter.type}_{$filter.id_key}"></a></div>
						<div class="accordion-body collapse in" id="div_layered_{$filter.type}_{$filter.id_key}">
							<div class="accordion-body-wrapper">
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}									
									<div class="{if isset($filter.is_color_group) && $filter.is_color_group}filter-color-container{else}{if $filter.type != 'category' && $filter.type != 'id_feature' && $filter.type != 'quantity' && $filter.type != 'condition' && $filter.type != 'manufacturer'}filter-size-container{else}filter-other-container filter-{$filter.type}-container{/if}{/if}">
										<div class="row">
											<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
												{foreach from=$filter.values key=id_value item=value name=fe}
													...
													<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
														...
														{else}
															<input type="checkbox" class="checkbox hidden" name="layered_{$filter.type_lite}_{$id_value}" id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}"{if isset($value.checked)} checked="checked"{/if}{if !$value.nbr} disabled="disabled"{/if} /> 
														{/if}
														<label for="layered_{$filter.type_lite}_{$id_value}"
															{if !$value.nbr} 
																class="disabled"
															{else}
																{if isset($filter.is_color_group) && $filter.is_color_group} 
															...
															{else}
																{if isset($filter.is_color_group) && $filter.is_color_group} 
																	<a class="filter-color-box {if isset($value.checked)} checked {/if}" style="background: {if isset($value.color)}{if file_exists($smarty.const._PS_ROOT_DIR_|cat:"/img/co/$id_value.jpg")}url(img/co/{$id_value}.jpg){else}{$value.color}{/if}{else}#CCC{/if};" data-rel="{$id_value}_{$filter.id_key}" href="{$value.link}"{if $value.rel|trim != ''} data-rel="{$value.rel}"{/if}>{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
																{else}
																	{if $filter.type != 'category' && $filter.type != 'id_feature' && $filter.type != 'quantity' && $filter.type != 'condition' && $filter.type != 'manufacturer'}
																		<span class="filter-size-box {if isset($value.checked)} active {/if}"> {$value.name|escape:'html':'UTF-8'}</span>
																	{else}
								...
								{else}
									...
									{else}
										<div class="filter-color-container">
											<div class="row">
												<div id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
								...
								{if $filter.filter_type == 0}
									{if $filter.type == 'price'}
                                         ...
                                         <div  class="filter-price">											
												<div id="ul_layered_{$filter.type}_{$filter.id_key}" class="layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
